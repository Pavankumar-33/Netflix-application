...
const Row = ({title, category, rowID}) => {
    const [movies, setMovies] = useState([])
    
    useEffect(() => {
        axios.get(`/java/get-movie/${category}`).then((response) => {
            console.log("Fashan",response)
  ...
  return (
    <>
        ...
        <div className='relative flex items-center group'>
            ...
            <div id={'slider'+rowID} className='w-full h-full overflow-x-scroll whitespace-nowrap scroll-smooth scorollbar-hide relative'>
                {
                    movies.map((item, id) => (
